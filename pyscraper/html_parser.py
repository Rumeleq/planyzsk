...
def parse_classroom_json_to_html(
    json_filename: str,
    o_map: dict[str, str],
    n_map: dict[str, str],
    s_map: dict[str, str],
    ...
    with open(
        f"{JSON_PATH}/timetables/classrooms/{json_filename}.json", "r", encoding="utf-8"
    ) as f:
    ...
    for i, week_day in enumerate(schedule):
        for j, school_hour in enumerate(schedule[week_day]):
            ...
            for length, grade in enumerate(grades):
                ...
                grade_anchor = soup.new_tag(
                    "a", class_="o", href=f"{o_map[base_grade]}.html"
                )
...
def parse_teacher_json_to_html(
    json_filename: str,
    o_map: dict[str, str],
    n_map: dict[str, str],
    s_map: dict[str, str],
    ...
    with open(
        f"{JSON_PATH}/timetables/teachers/{json_filename}.json", "r", encoding="utf-8"
    ) as f:
        schedule: dict[str, dict[str, list[tuple[str, str, str]]]] = json.load(f)
    ...
    if teacher_name == "K. Giżyński":
        twitch_link = soup.new_tag(
            "a",
            href="https://m.twitch.tv/kumi_gaming/home",
            id="kumi_gaming",
            style="margin-top: 20px; text-decoration: underline",
        )
...
def parse_grade_json_to_html(
    ...
    with open(
        f"{JSON_PATH}/timetables/grades/{json_filename}.json", "r", encoding="utf-8"
    ) as f:
    ...
    for i, week_day in enumerate(schedule):
        for j, school_hour in enumerate(schedule[week_day]):
            ...
            for k, lesson in enumerate(school_hour):
                ...
                anchor_n = soup.new_tag("a", class_="n", href=f"{n_map[teacher]}.html")
                anchor_n.string = f" {teacher} "
                anchor_s = soup.new_tag(
                    "a", class_="s", href=f"{s_map[classroom]}.html"
                )
                anchor_s.string = classroom

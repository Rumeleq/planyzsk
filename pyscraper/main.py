...
from html_parser import (
    parse_classroom_json_to_html,
    parse_grade_json_to_html,
    parse_teacher_json_to_html,
)
...
def insert_data_to_teachers(
    lesson_title: str,
    lesson_teacher: str,
    lesson_classroom: str,
    num_col: int,
    num_row: int,
    grade: str,
) -> None:
    ...
    if (
        lesson_teacher not in TEACHERS_TIMETABLES
    ):  # if teacher is not in the TEACHERS_TIMETABLES dictionary, add him
        TEACHERS_TIMETABLES[lesson_teacher] = {
            day: [None for _ in range(LESSONS_NUMBER)] for day in range(WEEK_DAYS)
        }  # add LESSONS_NUMBER lessons per day

    if not TEACHERS_TIMETABLES[lesson_teacher][num_col][
        num_row
    ]:  # if the lesson is empty, put it there
        TEACHERS_TIMETABLES[lesson_teacher][num_col][num_row] = (
            [grade],
            lesson_title,
            lesson_classroom,
        )
        ...
        TEACHERS_TIMETABLES[lesson_teacher][num_col][num_row][1] == lesson_title
        and TEACHERS_TIMETABLES[lesson_teacher][num_col][num_row][2] == lesson_classroom
    ):  # if the lesson is the same as the new one, just add the grade
        i = 0

        while (
            i < len(TEACHERS_TIMETABLES[lesson_teacher][num_col][num_row][0])
            and TEACHERS_TIMETABLES[lesson_teacher][num_col][num_row][0][i] < grade
        ):  # find the place where to put the grade
            i += 1
        TEACHERS_TIMETABLES[lesson_teacher][num_col][num_row][0].insert(
            i, grade
        )  # insert the grade
    else:
        raise ValueError(
            f"Error: {TEACHERS_TIMETABLES[lesson_teacher][num_col][num_row]} != {grade} {lesson_title} {lesson_classroom}"
        )  # if the lesson is different, raise an error


def insert_data_to_classrooms(
    lesson_title: str,
    lesson_teacher: str,
    lesson_classroom: str,
    num_col: int,
    ...
    if re.search(GROUP_REGEX, lesson_title) is not None:
        ...
        lesson_title = lesson_title[:-4]
    if (
        lesson_classroom not in CLASSROOMS_TIMETABLES
    ):  # if classroom is not in the CLASSROOMS_TIMETABLES dictionary, add it
        CLASSROOMS_TIMETABLES[lesson_classroom] = {
            day: [None for _ in range(LESSONS_NUMBER)] for day in range(WEEK_DAYS)
        }  # add LESSONS_NUMBER lessons per day

    if not CLASSROOMS_TIMETABLES[lesson_classroom][num_col][
        num_row
    ]:  # if the lesson is empty, put it there
        CLASSROOMS_TIMETABLES[lesson_classroom][num_col][num_row] = (
            lesson_teacher,
            [grade],
            lesson_title,
        )
        ...
        CLASSROOMS_TIMETABLES[lesson_classroom][num_col][num_row][2] == lesson_title
        and CLASSROOMS_TIMETABLES[lesson_classroom][num_col][num_row][0]
        == lesson_teacher
    ):  # if the lesson is the same as the new one, just add the grade
        i = 0
        while (
            i < len(CLASSROOMS_TIMETABLES[lesson_classroom][num_col][num_row][1])
            and CLASSROOMS_TIMETABLES[lesson_classroom][num_col][num_row][1][i] < grade
        ):  # find the place where to put the grade
            i += 1
        CLASSROOMS_TIMETABLES[lesson_classroom][num_col][num_row][1].insert(
            i, grade
        )  # insert the grade

    else:
        raise ValueError(
            f"Error: {CLASSROOMS_TIMETABLES[lesson_classroom][num_col][num_row]} != {grade} {lesson_teacher} {lesson_title}"
        )  # if the lesson is different, raise an error


def insert_data_to_grades(
    lesson_title: str,
    lesson_teacher: str,
    lesson_classroom: str,
    num_col: int,
    ...
    """
    if (
        grade not in GRADES_TIMETABLES
    ):  # if grade is not in the GRADES_TIMETABLES dictionary, add it
        GRADES_TIMETABLES[grade] = {
            day: [None for _ in range(LESSONS_NUMBER)] for day in range(WEEK_DAYS)
        }  # add LESSONS_NUMBER lessons per day

    if not GRADES_TIMETABLES[grade][num_col][
        num_row
    ]:  # if the lesson is empty, put it there
        GRADES_TIMETABLES[grade][num_col][num_row] = [
            (lesson_title, lesson_teacher, lesson_classroom)
        ]

    else:
        GRADES_TIMETABLES[grade][num_col][num_row].append(
            (lesson_title, lesson_teacher, lesson_classroom)
        )
...
async def get_timetable(session: ClientSession, i: int) -> None:
    ...
    async with session.get(f"{URL}o{i}.html") as response:  # get the timetable
        ...
        grade = timetable_html.find("span", class_="tytulnapis").text.split(" ")[
            0
        ]  # get the grade
        ...
        for td in timetable_html.find_all("td", class_="l"):
            ...
            for i, result in enumerate(group_regex_td_result):
                ...
                if (
                    group_regex_td_result is not None
                    and group_regex_span_result is False
                ):
                    spans_in_td[i].string += result

        row: Tag
        for row_num, row in enumerate(
            timetable_html.find("table", class_="tabela").find_all("tr")[1:]
        ):  # iterate over the lesson numbers (rows)
            col: Tag
...
def write_teacher_map_sorted_by_last_name_to_json(
    teachers_filenames: list[str],
) -> None:
    sorted_teachers_dict = dict(
        sorted(TEACHER_INTIAL_NAME_DICT.items(), key=lambda x: x[1].split(". ")[1])
    )
...
async def main():
    ...
    with open(f"{JSON_PATH}/resources/plain_text.json", "w", encoding="utf-8") as f:
        json.dump(
            PLAIN_TEXT, f, ensure_ascii=False, indent=4, sort_keys=True
        )  # save the PLAIN_TEXT dictionary to the file (used for creating PLAIN_TEXT_SOLUTION in other program)

    grades_json_files = [
        f"{JSON_PATH}timetables/grades/{file}".split("/")[-1].split(".")[0]
        for file in os.listdir(f"{JSON_PATH}timetables/grades/")
    ]
    teachers_json_files = [
        f"{JSON_PATH}timetables/teachers/{file}".split("/")[-1].split(".")[0]
        for file in os.listdir(f"{JSON_PATH}timetables/teachers/")
    ]
    classrooms_json_files = [
        f"{JSON_PATH}timetables/classrooms/{file}".split("/")[-1].split(".")[0]
...
def clear_output_files():
    folder_paths = [
        "../dane/",
        f"{JSON_PATH}timetables/grades/",
        f"{JSON_PATH}timetables/teachers/",
        f"{JSON_PATH}timetables/classrooms/",
    ]
...
if __name__ == "__main__":
    ...
    TEACHERS_TIMETABLES: dict[str, dict[str, list[tuple[list[str], str, str]]]] = (
        dict()
    )  # Variable to store teachers timetables {teacher: {day: [lesson1, lesson2, ...]}}
    CLASSROOMS_TIMETABLES: dict[str, dict[str, list[tuple[str, list[str], str]]]] = (
        dict()
    )  # Variable to store classrooms timetables {classroom: {day: [lesson1, lesson2, ...]}}
    GRADES_TIMETABLES: dict[str, dict[str, list[list[tuple[str, str, str]]]]] = (
        dict()
    )  # Variable to store grades timetables {grade: {day: [lesson1, lesson2, ...]}}
    PLAIN_TEXT: dict[str, dict[str, dict[str, str]]] = (
        dict()
    )  # Variable to store plain text lessons (later exported and used in other program to get PLAIN_TEXT_SOLUTION)

...
async def save_timetables(
    timetables: dict[str, dict[str, list[tuple[str, str, str]]]],
    directory: str,
    ...
    for timetable_name in timetables:
        tasks.append(
            asyncio.create_task(
                save_timetable(timetable_name, timetables[timetable_name], directory)
            )
        )
...
async def save_timetable(timetable_name: str, timetable, directory: str) -> None:
    ...
    async with aiofiles.open(
        f"{directory}{timetable_name}.json", "w", encoding="utf-8"
    ) as f:
        await f.write(
            json.dumps(timetable, ensure_ascii=False, indent=4)
